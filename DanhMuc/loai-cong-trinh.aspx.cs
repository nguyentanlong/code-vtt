...
{
    public partial class loai_cong_trinh : BasePage
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(loai_cong_trinh));
        ...
        public static object GetChaOptions(long excludeId)
        {
            ...
            try
            {
                ...
                var pars = new Dictionary<string, object> { { "@CongTyID", congTyId }, { "@ExcludeID", excludeId } };
                DataSet ds = db.ExecuteDatasetStoredProcedure("sp_long_DMLoaiCongTrinh_GetChaOptions", pars);
        ...
        public static object GetList(string keyword, string trangThai)
        {
            ...
            try
            {
                ...
                DataSet ds = db.ExecuteDatasetStoredProcedure("sp_long_DMLoaiCongTrinh_GetList", pars);
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    list.Add(new
                    {
                        LoaiCongTrinhID = dr["LoaiCongTrinhID"],
                        MaLoai = dr["MaLoai"].ToString(),
                        TenLoai = dr["TenLoai"].ToString(),
                        LoaiChaID = dr["LoaiChaID"] == DBNull.Value ? (object)null : dr["LoaiChaID"],
                        TenLoaiCha = dr["TenLoaiCha"] == DBNull.Value ? "" : dr["TenLoaiCha"].ToString(),
                        ThuTu = dr["ThuTu"],
        ...
        public static object GetById(long id)
        {
            ...
            {
                ConnectServer db = new ConnectServer();
                var pars = new Dictionary<string, object> { { "@LoaiCongTrinhID", id } };

                DataSet ds = db.ExecuteDatasetStoredProcedure("sp_long_DMLoaiCongTrinh_GetById", pars);
        ...
        [WebMethod(EnableSession = true)]
        public static object SaveData(long loaiCongTrinhId, string maLoai, string tenLoai, object loaiChaId, int thuTu, byte trangThai)
        {
            if (!IsAuthenticated())
                return new { success = false, message = "Phiên làm việc đã hết hạn. Vui lòng đăng nhập lại!" };
            ...
            log.Info($"SaveData called with loaiCongTrinhId: {loaiCongTrinhId}, congTyId: {congTyId}, maLoai: {maLoai}, tenLoai: {tenLoai}");
            try
            {
                ConnectServer db = new ConnectServer();
                var pars = new Dictionary<string, object>
                {
                    ...
                    { "@LoaiChaID", loaiChaId == null ? DBNull.Value : (object)Convert.ToInt64(loaiChaId) },
                    { "@ThuTu", thuTu },
                    { "@TrangThai", trangThai }
                };

                db.ExecuteDatasetStoredProcedure("sp_long_DMLoaiCongTrinh_Save", pars);
                return new { success = true, message = loaiCongTrinhId == 0 ? "Thêm mới thành công!" : "Cập nhật thành công!" };
            }
            catch (Exception ex)
            {
        ...
        public static object DeleteData(long id)
        {
            ...
            try
            {
                ...
                db.ExecuteDatasetStoredProcedure("sp_long_DMLoaiCongTrinh_Delete", pars);
