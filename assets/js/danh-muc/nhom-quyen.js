document.addEventListener("DOMContentLoaded", function () {
    loadData();
});

function callWebMethod(methodName, dataObj, successCallback) {
    fetch("nhom-quyen.aspx/" + methodName, {
        method: "POST",
        headers: { "Content-Type": "application/json; charset=utf-8" },
        body: JSON.stringify(dataObj || {})
    })
        .then(response => response.json())
        .then(res => {
            var result = res.d;
            if (result && result.success) {
                successCallback(result);
            } else {
                var errorMsg = result ? result.message : "Thao tác thất bại!";
                alert("Lỗi: " + errorMsg);
                if (errorMsg && (errorMsg.includes("đăng nhập") || errorMsg.includes("hết hạn"))) {
                    var currentUrl = encodeURIComponent(window.location.href);
                    window.location.href = "../Login.aspx?returnUrl=" + currentUrl;
                }
            }
        })
        .catch(err => {
            console.error("AJAX Error:", err);
            alert("Lỗi kết nối máy chủ hoặc hệ thống không phản hồi!");
        });
}

function loadData() {
    var keyword = document.getElementById("txtSearchKeyword").value;
    var trangThai = document.getElementById("ddlSearchTrangThai").value;

    callWebMethod("GetList", { keyword: keyword, trangThai: trangThai }, function (res) {
        var tbody = document.getElementById("tbodyNhomQuyen");
        tbody.innerHTML = "";

        if (!res.data || res.data.length === 0) {
            tbody.innerHTML = '<tr><td colspan="7" style="text-align:center; color:#888;">Không tìm thấy dữ liệu nào</td></tr>';
            return;
        }

        res.data.forEach(function (item, index) {
            var badgeClass = item.TrangThai === 1 ? "badge-success" : "badge-danger";
            var statusText = item.TrangThai === 1 ? "Đang hoạt động" : "Ngừng hoạt động";

            var tr = document.createElement("tr");
            tr.innerHTML = `
                <td style="text-align:center;">${index + 1}</td>
                <td><strong>${escapeHtml(item.MaNhomQuyen)}</strong></td>
                <td>${escapeHtml(item.TenNhomQuyen)}</td>
                <td style="text-align:center;">${item.ThuTu}</td>
                <td><span class="badge ${badgeClass}">${statusText}</span></td>
                <td>${item.NgayTaoText}</td>
                <td style="text-align:center;">
                    <button type="button" class="btn-icon text-edit" onclick="openModal(${item.NhomQuyenID})" title="Chỉnh sửa">
                        <i class="fa fa-edit"></i> Sửa
                    </button>
                    <button type="button" class="btn-icon text-delete" onclick="deleteData(${item.NhomQuyenID})" title="Xóa">
                        <i class="fa fa-trash"></i> Xóa
                    </button>
                </td>
            `;
            tbody.appendChild(tr);
        });
    });
}

function openModal(id) {
    document.getElementById("hddNhomQuyenID").value = id;

    if (id === 0) {
        document.getElementById("modalTitle").innerText = "Thêm mới Nhóm quyền";
        document.getElementById("txtMaNhomQuyen").value = "";
        document.getElementById("txtMaNhomQuyen").readOnly = false;
        document.getElementById("txtTenNhomQuyen").value = "";
        document.getElementById("txtThuTu").value = "0";
        document.getElementById("ddlTrangThai").value = "1";
        document.getElementById("modalNhomQuyen").style.display = "flex";
    } else {
        document.getElementById("modalTitle").innerText = "Chỉnh sửa Nhóm quyền";
        callWebMethod("GetById", { id: id }, function (res) {
            var d = res.data;
            document.getElementById("txtMaNhomQuyen").value = d.MaNhomQuyen;
            document.getElementById("txtMaNhomQuyen").readOnly = true;
            document.getElementById("txtTenNhomQuyen").value = d.TenNhomQuyen;
            document.getElementById("txtThuTu").value = d.ThuTu;
            document.getElementById("ddlTrangThai").value = d.TrangThai;
            document.getElementById("modalNhomQuyen").style.display = "flex";
        });
    }
}

function closeModal() {
    document.getElementById("modalNhomQuyen").style.display = "none";
}

function saveData() {
    var id = parseInt(document.getElementById("hddNhomQuyenID").value);
    var maNhomQuyen = document.getElementById("txtMaNhomQuyen").value.trim();
    var tenNhomQuyen = document.getElementById("txtTenNhomQuyen").value.trim();

    if (!maNhomQuyen) {
        alert("Vui lòng nhập Mã nhóm quyền!");
        document.getElementById("txtMaNhomQuyen").focus();
        return;
    }
    if (!tenNhomQuyen) {
        alert("Vui lòng nhập Tên nhóm quyền!");
        document.getElementById("txtTenNhomQuyen").focus();
        return;
    }

    var payload = {
        nhomQuyenId: id,
        maNhomQuyen: maNhomQuyen,
        tenNhomQuyen: tenNhomQuyen,
        thuTu: parseInt(document.getElementById("txtThuTu").value) || 0,
        trangThai: parseInt(document.getElementById("ddlTrangThai").value)
    };

    callWebMethod("SaveData", payload, function (res) {
        alert(res.message);
        closeModal();
        loadData();
    });
}

function deleteData(id) {
    if (confirm("Bạn có chắc chắn muốn xóa Nhóm quyền này khỏi hệ thống? (Xóa vĩnh viễn, không thể khôi phục)")) {
        callWebMethod("DeleteData", { id: id }, function (res) {
            alert(res.message);
            loadData();
        });
    }
}

function escapeHtml(text) {
    if (!text) return "";
    return text
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")
        .replace(/"/g, "&quot;")
        .replace(/'/g, "&#039;");
}